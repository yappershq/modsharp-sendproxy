...
internal static unsafe class UniformEncoderHook
{
    ...
    private static nint SharedHook(nint bf, nint fieldInfo, nint paramsPtr, nint valuePtr, uint extra)
    {
        ...
        try
        {
            ...
            if (!ProxyRegistry.IsEmpty && EncodeCapture.Active && _entityManager is { } em)
            {
                var entIdx = EncodeCapture.EntityIndex;
                var serPtr = EncodeCapture.Serializer;
                if (entIdx >= 0 && NativeUtil.IsUserPtr(serPtr))
                {
                    var serName = NativeUtil.ReadShortAscii(*(nint*) serPtr, 48);
                    var field   = NativeUtil.ReadFieldName(fieldInfo);
                    if (serName.Length > 0 && field.Length > 0
                        && ProxyRegistry.TryGet(serName, field, entIdx, out var proxy))
                    {
