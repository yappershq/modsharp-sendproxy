...
internal static class PerClientDispatch
{
    ...
    private static readonly ConcurrentDictionary<(int ent, string field), Rec> _table = new();
    ...
    public static void Record(int entityIndex, string field, FieldType type, in SpoofValue def,
        List<(IGameClient Client, SpoofValue Value)> overrides)
    {
        if (entityIndex < 0 || field.Length == 0 || overrides.Count == 0)
        {
            return;
        }
    ...
    public static bool TryResolve(int entityIndex, string field, int slot, out SpoofValue value, out FieldType type)
    {
        value = default;
